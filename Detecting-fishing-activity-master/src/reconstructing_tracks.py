...
def plot_track_predictions(df,mod_num,mmsi,pickeled_model,up,down,left,right):
	...
	fig_title = f"Gear: {df['gear'].iloc[0] if 'gear' in df.columns else 'Unknown'} | Model: {mod_num} | MMSI: {mmsi}"
	...
	m.drawparallels(np.arange(np.floor(min_lat), np.ceil(max_lat), (max_lat-min_lat)/4), labels=[1,0,0,0], linewidth=0.2)
	m.drawmeridians(np.arange(np.floor(min_lon), np.ceil(max_lon), (max_lon-min_lon)/4), labels=[0,0,0,1], linewidth=0.2)
...
if __name__ == '__main__':
	...
	for gear in gears:
		if gear == 'longliners':
			mod_num = 'model_6'
			pickeled_model = '../results/model_6_RF_longliners.pkl'
			...
			mod_num = 'model_3'
			pickeled_model = '../results/model_3_RF_trawlers.pkl'
			...
			mod_num = 'model_3'
			pickeled_model = '../results/model_3_GBC_purse_seines.pkl'
